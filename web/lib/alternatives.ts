...
function alternativesHero(bySlug: (slug: string) => Alternative): string {
  const groups = [
    { label: "Visible surfaces", slugs: ["droppy", "notchnook", "alcove"] },
    { label: "Information & launch", slugs: ["dynamiclake", "boring-notch", "raycast"] },
    { label: "Focused specialists", slugs: ["rectangle", "maccy", "alttab"] },
    { label: "System control", slugs: ["flux", "karabiner-elements", "bettertouchtool"] },
  ]
  let rowIndex = 0
  const index = groups
    .map((group) => {
      const rows = group.slugs
        .map((slug) => {
          ...
          const row = rowIndex++
          return `<a class="alt-index-row" href="${alternative.slug}/" style="--row:${row}">${altMarkHtml(alternative.slug, alternative.name)}<span class="alt-index-copy"><strong>${escapeHtml(alternative.name)}</strong><small>${escapeHtml(alternative.category)}</small></span><span class="alt-index-arrow" aria-hidden="true">→</span></a>`
        })
        .join("")
      return `<section class="alt-index-group"><h2>${group.label}</h2><div>${rows}</div></section>`
    })
    .join("")

  return `<div class="article-top alternatives-hub-top alternative-shell">${navigation()}<main><section class="article-hero alternatives-hub-hero"><div class="wrap alternatives-hero-grid"><div class="alternatives-hero-copy"><p class="alternatives-kicker">Mac utilities · ${data.alternatives.length} compared</p><h1>Find the Mac utility that fits your day.</h1><p class="article-summary">Notch tools, launchers, window managers, clipboard apps, and automation layers—compared by what they actually help you do.</p><div class="alternatives-hero-actions"><a class="btn" href="https://downloads.trygojo.com/Gojo.dmg">Try Gojo free</a><a href="#alternatives-library">Compare all twelve <span aria-hidden="true">↓</span></a></div><p class="alternatives-hero-trust">Three days free · No account · macOS 14+</p></div><div class="alternatives-index" aria-label="Mac utilities by job">${index}</div></div></section></main></div>`
}
...
export function alternativeHubBody() {
  ...
  const hero = alternativesHero(bySlug)
  return `${hero}
