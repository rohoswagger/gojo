...
@MainActor
class GojoViewCoordinator: ObservableObject {
    static let shared = GojoViewCoordinator()
    ...
    private init() {
        ...
        Task { @MainActor in
            // The hello animation no longer runs at launch (it competed with the
            // welcome screen). It now plays as the final onboarding beat, triggered
            // from the onboarding window's onFinish in GojoApp.swift.
