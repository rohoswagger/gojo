...
private struct IdentityRow: View {
    ...
    var body: some View {
        HStack(spacing: 9) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                // Always one info line below the name — error replaces the position
                // readout in the same slot rather than stacking a new line, so the row
                // height never changes (no layout shift when an error appears).
                secondaryLine
            }
    ...
    private var secondaryLine: some View {
        if state.statusKind == .error {
            // On its own line the message fits at full size; minimumScaleFactor only
            // kicks in for unusually long text, so it stays readable without truncating.
            Text(errorMessage)
