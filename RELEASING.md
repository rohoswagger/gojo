...
> `.env.local`. Downloaders must clear quarantine before first launch:
...
4. **Cut the release.** Takes 5–15 minutes (notarization is the slow part):
   ...
   4. Staple the ticket
   5. Build the styled DMG (custom background + icon layout via `dmgbuild`, run from an isolated, hash-pinned venv at `.build/dmg-venv` built from `Configuration/dmg/requirements.txt`), then sign it
