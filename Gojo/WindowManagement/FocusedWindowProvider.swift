...
final class FocusedWindowProvider {
    ...
    func setFrame(_ normalFrame: CGRect, for window: FocusedWindow) async -> Bool {
        guard let element = window.element else {
            ...
            let result = await XPCHelperClient.shared.setWindowFrame(normalFrame, pid: window.pid, windowID: window.windowID)
            gojoDebug("setFrame[helper] \(window.appName) windowID=\(String(describing: window.windowID)) → \(result.success ? "ok" : "FAILED") frame=\(normalFrame)")
